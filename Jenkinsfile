        ...
        stage('Ruff Lint') {
            steps {
                script {
                    ...
                    try {
                        sh "docker compose run --name market_ruff_${env.BUILD_NUMBER} ${env.APP_SERVICE} uv run ruff check --output-file=${env.REPORTS_DIR}/ruff-log.txt src tests"
                    } finally {
        ...
        stage('MyPy Type Check') {
            steps {
                script {
                    ...
                    try {
                        sh "docker compose run --name market_mypy_${env.BUILD_NUMBER} ${env.APP_SERVICE} bash -c 'set -o pipefail && uv run mypy src | tee ${env.REPORTS_DIR}/mypy-log.txt'"
                    } finally {
        ...
        stage('Execute Pytest Suite') {
            steps {
                script {
                    ...
                    try {
                        sh "docker compose run --name market_pytest_${env.BUILD_NUMBER} -e TEST_DATABASE_URL=${env.TEST_DATABASE_URL} -e DATABASE_URL=${env.TEST_DATABASE_URL} ${env.APP_SERVICE} uv run pytest --junitxml=${env.REPORTS_DIR}/junit.xml --cov-report=xml:${env.REPORTS_DIR}/coverage.xml --cov-report=html:${env.REPORTS_DIR}/htmlcov"
                    } finally {
